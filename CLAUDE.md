...
- **Canonical destination theme**: `botanique-horizon-preview` ID `#195678142804` on store `a9iz0x-ip.myshopify.com` (decided 2026-05-05f). Every `shopify theme pull` and `shopify theme push` issued by operator or loop MUST pass `--theme=195678142804` explicitly. NEVER `--live` (5d Zest-pollution lesson). NEVER target the development theme `#195371860308` for canonical work — that's per-machine ephemeral. NEVER target Zest `#178646581588` — that's the live store theme on the same Shopify account, unrelated to this clone, and a same-store collision is the exact 5d failure mode.
- **Anti-target themes (do NOT pull/push to these)**: `Zest #178646581588` (live, different brand), 6 unpublished themes literally named `195371860308` on store `a9iz0x-ip` (IDs `#195673555284`, `#195677978964`, `#195683811668`, `#195683844436`, `#195683877204`, `#195684204884`, `#195684270420`) — artifacts of past `theme dev` misnamings; cleanup candidates after Phase 0 lands but not before.
